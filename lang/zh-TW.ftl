...
luducard-cloud-details-title = 帳戶詳情
luducard-cloud-provider = 已連接提供商:
luducard-cloud-email = 帳戶郵箱:
luducard-cloud-destination = 目標資料夾:
luducard-cloud-engine = 備份引擎:
luducard-cloud-path = 可執行文件路徑:
luducard-cloud-rules-title = 同步規則
luducard-cloud-upload-auto = 自動上傳
luducard-cloud-upload-auto-desc = 本地備份生成後立即上傳存檔。
luducard-cloud-download-latest = 下載最新版本
luducard-cloud-download-latest-desc = 優先考慮雲端較新的文件。
luducard-cloud-toast-upload-enabled = 自動上傳已啟用
luducard-cloud-toast-upload-disabled = 自動上傳已禁用
luducard-cloud-toast-download-enabled = 自動下載已啟用
luducard-cloud-toast-download-disabled = 自動下載已禁用
luducard-support-dest = 資源分配
luducard-support-dest-desc = 所有貢獻均完全用於雲端伺服器維護（託管和數據流量）。
luducard-danger-zone = 危險區域
luducard-danger-zone-desc = 無法撤銷的破壞性操作。
luducard-reset-warning-title = 重要警告:
luducard-reset-warning-desc = 這將永久擦除所有設置、自定義搜索路徑、遊戲掃描歷史記錄、雲憑據和保存配置文件。磁盤上的原始備份文件夾不會被刪除。
luducard-btn-reset-app = 清除所有數據
luducard-btn-reset-confirm = 是的，確認刪除
